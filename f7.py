class f7:
    def demo(self):
        ...
        with open('iris.data','r')as open_file:
            iris=open_file.read()
            iris=iris.strip()
            iris=re.split('[\n,]',iris)
            for i in range(len(iris)):
                if iris[i]=='Iris-setosa':
                    iris[i]='0'
                elif iris[i]=='Iris-versicolor':
                    iris[i]='1'
                elif iris[i]=='Iris-virginica':
                    iris[i]='2'
            iris=[iris[i:i+5] for i in range(0,len(iris),5)]
            iris=np.array(iris,dtype=float)
            X=np.delete(iris,[4],axis=1)
            y=iris.T[4]
            ...
            # L1
            lsvc = LinearSVC(C=0.009, penalty="l1", dual=False)
            lsvc.fit(X, y)
            ...
            # 0.666666666667 0.333333333333 0.666666666667 0.333333333333
