...
class hhh2:
    def demo(self):
        with open('wdbc.data','r')as open_file:
            wdbc=open_file.read()
            wdbc=wdbc.strip()
            wdbc=re.split('[\n,]',wdbc)
            for i in range(len(wdbc)):
                if wdbc[i]=='M':
                    wdbc[i]='0'
                elif wdbc[i]=='B':
                    wdbc[i]='1'
            wdbc=[wdbc[i:i+32] for i in range(0,len(wdbc),32)]
            wdbc=np.array(wdbc,dtype=float)
            X=np.delete(wdbc,[0],axis=1)
            y=wdbc.T[0]
        ...
        # 0.00527240773286 0.00527240773286 0.00527240773286 0.00175746924429
        #0.00527240773286 0.00527240773286 0.00527240773286 0.00351493848858
